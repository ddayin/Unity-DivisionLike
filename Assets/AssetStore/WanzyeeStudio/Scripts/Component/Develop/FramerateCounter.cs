...
namespace WanzyeeStudio{
	...
	public class FramerateCounter : BaseSingleton<FramerateCounter>{
		...
		private static bool CheckValid(bool must){
			...
			if(null == _instance && must) Debug.LogError("There's no FramerateCounter instance.");
			if(null == _instance) return false;

			if(!_instance.enabled) _instance.enabled = true;
			if(_instance.isActiveAndEnabled) return true;

			if(must) Debug.LogError("FramerateCounter can't work with GameObject inactive in hierarchy.", _instance);
			return false;
